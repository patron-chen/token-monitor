...
- **New provider credentials** are declared as a field `storePath` in the provider's `src/shared/providers/<id>/account.js`; `CREDENTIAL_SETTING_PATHS` derives from those declarations, so a provider entry added directly to `credentialStore.js` would bypass the refresh-scope keys and renderer projection. Hub secrets and the network proxy password remain non-provider entries in the unified store. Proxy configured-state projection stays outside the provider projection; raw passwords stay in main. Dynamic accounts use a nested path in the same store — never a provider-specific store.
- **Settings completion** includes asynchronous proxy reconfiguration. Credential save and clear await `applySettingsPatch` before returning the renderer settings DTO or reporting success; a failed write or proxy application propagates through the IPC command.
